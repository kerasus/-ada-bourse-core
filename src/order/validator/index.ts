import type {
  ...
  CheckRestrictionParamType,
  RestrictionType,
  UserInstrumentType,
  CustomerAccountBalanceType,
  InstrumentCache,
  OrderType,
  WageType,
} from '../../types'
...
export function checkBuyingPower (
  ...
  const getAccountBalance = (accountType: AccountsTypeKeys): number => customerAccountBalances.find((account) => account.type === accountType)?.balance ?? 0
...
export function checkWealth (
  ...
  try {
    ...
    const instrument: InstrumentCache | null = orderInstrument
